...
const props = defineProps({ court: Object });

const form = useForm({
    _method: 'PUT',
    name: props.court.name,
    type: props.court.type,
    ...
    <AdminLayout>
        <template #header>
            <div class="flex items-center space-x-2">
                ...
                <h1 class="text-xl font-bold text-surface-900">Edit {{ court.name }}</h1>
            </div>
        </template>
            ...
            <div class="card p-6">
                <form @submit.prevent="submit" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input v-model="form.price_per_hour" type="number" class="input-field" min="0" />
                    ...
                    <div>
                        ...
                        <div v-if="court.image" class="mb-2">
                            <img :src="court.image_url || ('/storage/' + court.image)" :alt="court.name" class="w-32 h-20 object-cover rounded-lg" @error="$event.target.src = '/images/' + court.image" />
                        </div>
